...
public class TracksBrowser extends FragmentActivity implements
	...
	protected void onCreate(Bundle icicle) {
		...
		mPanel.setBackgroundDrawable(getResources().getDrawable(
	...
	public void onBackPressed() {
		...
		if (/* mPanel.isExpanded() */panelState.equals(PanelState.EXPANDED)) {
			// mPanel.collapsePane();
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case R.id.image_edit_file:
			...
			mImageProvider.loadImage(
					(ImageView) findViewById(R.id.half_artist_image), mInfo);
			return true;
		case R.id.image_edit_lastfm:
			...
			if (Audio.Artists.CONTENT_TYPE.equals(mimeType)) {
				...
				mInfo.type = TYPE_ALBUM;
				mInfo.data = new String[] { getAlbumId(), getArtist(),
						getAlbum() };
			}
			mImageProvider.loadImage(
					(ImageView) findViewById(R.id.half_artist_image), mInfo);
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == FragmentActivity.RESULT_OK
				&& requestCode == RESULT_LOAD_IMAGE && data != null) {
			...
			if (Audio.Artists.CONTENT_TYPE.equals(mimeType)) {
				...
				mInfo.type = TYPE_ALBUM;
				mInfo.data = new String[] { getAlbumId(), getAlbum(),
						getArtist(), picturePath };
			} else if (Audio.Playlists.CONTENT_TYPE.equals(mimeType)) {
				mInfo.type = TYPE_PLAYLIST;
				mInfo.data = new String[] { bundle.getString(PLAYLIST_NAME),
						picturePath };
			} else {
				Long id = bundle.getLong(BaseColumns._ID);
				mInfo.type = TYPE_GENRE;
				mInfo.data = new String[] {
						MusicUtils.parseGenreName(this,
								MusicUtils.getGenreName(this, id, true)),
						picturePath };
			}
	...
	private class PageListener extends SimpleOnPageChangeListener {
		public void onPageSelected(int cur) {
			if (cur == 0) {
				...
				TextView lineTwoView = (TextView) findViewById(R.id.half_artist_image_text_line_two);
				String lineTwo = MusicUtils.makeAlbumsLabel(TracksBrowser.this,
						Integer.parseInt(getNumAlbums()), 0, false);
				lineTwoView.setText(lineTwo);
			} else {
				ImageButton mCButton = (ImageButton) findViewById(R.id.view_more);
				...
				TextView lineTwoView = (TextView) findViewById(R.id.half_artist_image_text_line_two);
				String lineTwo = MusicUtils.makeAlbumsLabel(TracksBrowser.this,
						0, Integer.parseInt(getNumSongs()), true);
				lineTwoView.setText(lineTwo);
			}
		}
	}
	...
	private void setTitle() {
		...
		if (Audio.Playlists.CONTENT_TYPE.equals(mimeType)) {
			...
			id = bundle.getLong(BaseColumns._ID);
			name = getString(R.string.artist_page_title)
					+ MusicUtils.getArtistName(this, id, true);
		} else if (Audio.Albums.CONTENT_TYPE.equals(mimeType)) {
			id = bundle.getLong(BaseColumns._ID);
			name = getString(R.string.album_page_title)
					+ MusicUtils.getAlbumName(this, id, true);
		} else if (Audio.Genres.CONTENT_TYPE.equals(mimeType)) {
			id = bundle.getLong(BaseColumns._ID);
			name = MusicUtils.parseGenreName(this,
					MusicUtils.getGenreName(this, id, true));
		} else {
			setTitle(R.string.app_name);
			return;
